...
async function connectDB() {
    try {
        ...
        sampleItems.forEach(item => {
            console.log(`Startup Check - Item: ${item.name}, Price: ${item.price}, Quantity: ${item.quantity || 'N/A'}`);
        });
    } catch (err) {
...
const isAdmin = async (req, res, next) => {
    const email = req.headers["x-admin-email"] || req.query.email || req.body.email;
    if (req.method === 'GET' && !email) {
        console.warn("No email provided for GET request. Proceeding without authentication check.");
        return next();
    }
    if (!email) {
        return res.status(403).json({ success: false, message: "Access denied. Admin email is required." });
    }
...
app.post("/admin/login", async (req, res) => {
    ...
    res.json({ success: true, message: "Admin login successful!", email: user.email, name: user.name });
});
...
app.post("/admin/forgot", async (req, res) => {
    ...
    const user = await usersCollection.findOne({ email });
    if (!user || !user.isAdmin) return res.status(404).json({ success: false, message: "Admin email not found." });
...
app.get("/admin/menu", isAdmin, async (req, res) => {
    try {
        ...
        const formattedItems = menuItems.map(item => {
            const numericPrice = typeof item.price === 'number' ? item.price : parseFloat(item.price.replace(/[^0-9.]/g, '')) || 0;
            const numericQuantity = item.quantity !== undefined ? parseInt(item.quantity) : 0;
            return { 
                ...item, 
...
app.post("/admin/menu/add", isAdmin, async (req, res) => {
    ...
    if (!name || !category || !price || quantity === undefined) {
        return res.status(400).json({ success: false, message: "Missing required fields (name, category, price, quantity)." });
    }
    ...
    try {
        ...
        res.json({ success: true, message: "Menu item added successfully.", item: { ...menuItem, price: `₹${menuItem.price.toFixed(2)}` } });
    } catch (err) {
...
app.put("/admin/menu/update/:id", isAdmin, async (req, res) => {
    ...
    if (quantity !== undefined && !isNaN(quantity) && quantity >= 0) updateFields.quantity = parseInt(quantity);
    ...
    try {
        ...
        const numericPrice = typeof updatedItem.price === 'number' ? updatedItem.price : parseFloat(updatedItem.price.replace(/[^0-9.]/g, '')) || 0;
        res.json({ success: true, message: "Menu item updated successfully.", item: { ...updatedItem, price: `₹${numericPrice.toFixed(2)}` } });
    } catch (err) {
    ...
    try {
        ...
        res.json({ 
            ...
            item: { 
                ...
                price: `₹${(typeof updatedItem.price === 'number' ? updatedItem.price : parseFloat(updatedItem.price.replace(/[^0-9.]/g, '')) || 0).toFixed(2)}` 
            } 
        });
    } catch (err) {
...
app.post("/admin/tables/add", isAdmin, async (req, res) => {
    ...
    if (!name || !pricePerHour || !capacity || ac === undefined) {
        return res.status(400).json({ success: false, message: "Missing required fields (name, capacity, ac, pricePerHour)." });
    }
    if (available !== undefined && (isNaN(available) || available < 0)) {
        return res.status(400).json({ success: false, message: "Available must be a non-negative integer." });
    }
    const table = {
        name,
...
app.put("/admin/tables/update/:id", isAdmin, async (req, res) => {
    ...
    try {
        ...
        if (result.modifiedCount === 0) {
            return res.status(404).json({ success: false, message: "Table not found or no changes made." });
        }
        const updatedTable = await tableCollection.findOne({ _id: new ObjectId(id) });
        res.json({ success: true, message: "Table updated successfully.", table: updatedTable });
    } catch (err) {
...
app.post("/admin/eventhalls/add", isAdmin, async (req, res) => {
    ...
    if (!name || !capacity || !pricePerHour) {
        return res.status(400).json({ success: false, message: "Missing required fields (name, capacity, pricePerHour)." });
    }
    const eventHall = {
        name,
        capacity: parseInt(capacity),
    ...
    try {
        ...
        res.json({ success: true, message: "Event hall added successfully.", eventHall });
    } catch (err) {
        console.error("Error adding event hall:", err);
        res.status(500).json({ success: false, message: "Server error adding event hall." });
    }
});
...
app.put("/admin/eventhalls/update/:id", isAdmin, async (req, res) => {
    ...
    try {
        ...
        if (result.modifiedCount === 0) {
            return res.status(404).json({ success: false, message: "Event hall not found or no changes made." });
        }
        const updatedEventHall = await eventCollection.findOne({ _id: new ObjectId(id) });
        res.json({ success: true, message: "Event hall updated successfully.", eventHall: updatedEventHall });
    } catch (err) {
        console.error("Error updating event hall:", err);
        res.status(500).json({ success: false, message: "Server error updating event hall." });
    }
});
...
app.delete("/admin/eventhalls/delete/:id", isAdmin, async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "Server error deleting event hall." });
    }
});
...
app.post("/admin/offers/add", isAdmin, async (req, res) => {
    ...
    if (!itemId || !offerPrice) {
      return res.status(400).json({ success: false, message: "Missing required fields (itemId, offerPrice)." });
    }
    try {
      ...
      const rawPrice = menuItem.price !== undefined && menuItem.price !== null ? menuItem.price.toString() : "0";
      const originalPrice = Number(rawPrice.replace(/[^0-9.-]+/g, ""));
      if (isNaN(originalPrice) || originalPrice <= 0) {
        console.error(`Invalid original price for item ${menuItem.name}: ${rawPrice}, _id: ${menuItem._id}`);
        return res.status(400).json({ success: false, message: `Invalid original price for ${menuItem.name}` });
      }
      const offer = {
      ...
      res.status(500).json({ success: false, message: "Server error adding offer." });
    }
  });
...
app.put("/admin/offers/update/:id", isAdmin, async (req, res) => {
    ...
    if (!offerPrice) {
        return res.status(400).json({ success: false, message: "Offer price is required." });
    }
    try {
        ...
        if (result.modifiedCount === 0) {
            return res.status(404).json({ success: false, message: "Offer not found or no changes made." });
        }
        const updatedOffer = await offerCollection.findOne({ _id: new ObjectId(id) });
