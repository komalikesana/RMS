...
app.post("/forgot", async (req, res) => {
    ...
    try {
        ...
        console.log(`Debug - Code generated for ${email}: ${code}, Timestamp: ${timestamp}`);
...
app.post("/send-code", async (req, res) => {
    ...
    try {
        ...
        const timestamp = Date.now();
        verificationCodes[email] = { code, timestamp };
        console.log(`Debug - OTP generated for ${email}: ${code}, Timestamp: ${timestamp}`);
        await transporter.sendMail({
            from: process.env.EMAIL_USER,
            to: email,
...
app.post("/verify-code", async (req, res) => {
    ...
    try {
        ...
        if (!stored || stored.code !== code || Date.now() - stored.timestamp > 10 * 60 * 1000) {
    ...
    try {
        ...
        const menuWithOffers = menuItems.map(item => {
            ...
            if (isNaN(price) || price <= 0) {
                console.error(`Invalid price for item ${item.name}: ${rawPrice}, _id: ${item._id}, using 0 as fallback`);
                return { ...item, price: 0, offerPrice: null, quantity: item.quantity || 0 };
            }
...
app.get("/menu/name/:name", async (req, res) => {
    ...
    try {
      const item = await menuCollection.findOne({ name: new RegExp(`^${name}$`, 'i') }); // Case-insensitive match
      if (!item) return res.status(404).json({ message: "Menu item not found." });
      console.log(`Fetched item for name ${name}:`, item); // Debug log
      const rawPrice = item.price !== undefined && item.price !== null ? item.price.toString() : "0";
      const price = Number(rawPrice.replace(/[^0-9.-]+/g, ""));
      if (isNaN(price) || price <= 0) {
        console.error(`Invalid or missing price for item ${item.name}: ${rawPrice}, _id: ${item._id}`);
...
// Cart API with Support for Menu, Tables, and Event Halls
app.post("/cart/add", async (req, res) => {
    ...
    try {
      ...
      // Handle menu items (with stock management)
      if (itemType === "menu") {
        ...
        if (menuItem.quantity < quantity) {
          return res.status(400).json({ success: false, message: `Insufficient stock for ${menuItem.name}. Available: ${menuItem.quantity}` });
        }
  
        // Apply offer price if available
        const offer = await offerCollection.findOne({ itemId: new ObjectId(itemId) });
        if (offer && !isNaN(Number(offer.offerPrice)) && Number(offer.offerPrice) > 0) {
      ...
      // Handle tables
      // Inside the /cart/add endpoint, replace the table handling logic
        else if (itemType === "table") {
            const table = await tableCollection.findOne({ _id: new ObjectId(itemId) });
            if (!table) {
                return res.status(404).json({ success: false, message: "Table not found." });
            }
            if (table.available < quantity) {
                return res.status(400).json({ success: false, message: `Table ${table.name} has insufficient availability. Available: ${table.available}` });
            }
            await tableCollection.updateOne(
                { _id: new ObjectId(itemId) },
                { $inc: { available: -quantity } } // Decrement available count by quantity
            );
        }
      // Handle event halls
        ...
        if (!hall.available) {
          return res.status(400).json({ success: false, message: `Event hall ${hall.name} is not available.` });
        }
        ...
        return res.status(400).json({ success: false, message: `Invalid item type: ${itemType}` });
      }
      ...
      if (userCart) {
        const itemIndex = userCart.items.findIndex(item => item._id === itemId && item.itemType === itemType);
        if (itemIndex > -1) {
          userCart.items[itemIndex].quantity += quantity;
        } else {
          userCart.items.push({ _id: itemId, name, price: effectivePrice, quantity, itemType });
        }
        ...
        const newCart = {
          email,
          items: [{ _id: itemId, name, price: effectivePrice, quantity, itemType }],
      ...
      if (!updatedCart) {
        return res.status(500).json({ success: false, message: "Failed to retrieve updated cart" });
      }
  
      res.json({ success: true, message: "Item added successfully", cart: updatedCart });
    } catch (error) {
      console.error("Error in /cart/add:", error);
      // Rollback for menu items only
      if (itemType === "menu") {
        await menuCollection.updateOne(
          { _id: new ObjectId(itemId) },
          { $inc: { quantity: quantity } }
...
app.put("/cart/:email/:itemId", async (req, res) => {
    ...
    try {
        ...
        if (newQty <= 0) {
            // Restore availability for tables/event halls
            if (itemType === "table") {
                await tableCollection.updateOne(
                    { _id: new ObjectId(itemId) },
                    { $inc: { available: item.quantity } } // Restore available count by original quantity
                );
            } else if (itemType === "eventhall") {
                await eventCollection.updateOne(
                    { _id: new ObjectId(itemId) },
...
app.get("/offers", async (req, res) => {
    try {
        ...
        if (!offers || offers.length === 0) {
            console.log("No offers found in offerCollection");
            return res.json([]);
        }

        const offerItemNames = offers
            .filter(offer => offer.itemName && typeof offer.itemName === "string" && offer.itemName.trim())
        ...
        const menuItems = await menuCollection.find({ name: { $in: offerItemNames } }).toArray();
        console.log(`Fetched ${menuItems.length} menu items for ${offerItemNames.length} unique offer itemNames`);
        ...
        const enrichedOffers = offers.map(offer => {
            if (!offer.itemName || typeof offer.itemName !== "string" || !offer.itemName.trim()) {
            ...
            if (!menuItem) {
                console.warn(`No menu item found for offer itemName: ${offer.itemName}`);
            }
            ...
            try {
                if (typeof offer.originalPrice === "string") {
                    originalPrice = parseFloat(offer.originalPrice.replace(/[^0-9.]/g, "")) || 0;
                } else if (typeof offer.originalPrice === "number") {
                    originalPrice = offer.originalPrice;
                }

                if (typeof offer.offerPrice === "string") {
                    offerPrice = parseFloat(offer.offerPrice.replace(/[^0-9.]/g, "")) || 0;
                } else if (typeof offer.offerPrice === "number") {
                    offerPrice = offer.offerPrice;
                }
            } catch (e) {
                console.error(`Price parsing error for ${offer.itemName || "Unknown"}:`, e.message);
            }

            const imageUrl = menuItem && menuItem.image && typeof menuItem.image === "string" && menuItem.image.trim()
                ? menuItem.image
        ...
        const validOffers = enrichedOffers.filter(offer => offer !== null && offer.image !== "https://via.placeholder.com/100");
        console.log(`Returning ${validOffers.length} valid offers`);
        res.json(validOffers.length > 0 ? validOffers : enrichedOffers.filter(offer => offer !== null));
    } catch (err) {
        console.error("Error in /offers endpoint:", err.stack);
        res.status(500).json({ message: "Server error fetching offers", error: err.message });
    }
});
